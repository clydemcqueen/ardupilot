 ...
 * -- The sub is below RNGHOLD_DEPTH
 *
...
bool ModeRnghold::init(bool ignore_checks)
{
    ...
    } else if (sub.inertial_nav.get_position_z_up_cm() >= sub.g.rnghold_depth) {
        sub.gcs().send_text(MAV_SEVERITY_WARNING, "descend below %f meters to hold range", sub.g.rnghold_depth * 0.01f);
    }
...
bool ModeRnghold::set_rangefinder_target_cm(float target_cm)
{
    ...
    if (sub.control_mode != Number::RNG_HOLD) {
        ...
        sub.gcs().send_text(MAV_SEVERITY_WARNING, "descend below %f meters to set rangefinder target", sub.g.rnghold_depth * 0.01f);
...
void ModeRnghold::update_surface_offset()
{
    if (sub.rangefinder_alt_ok()) {
        ...
        if (!HAS_VALID_TARGET && sub.rangefinder_state.inertial_alt_cm < sub.g.rnghold_depth) {
            set_rangefinder_target_cm(sub.rangefinder_state.inertial_alt_cm - rangefinder_terrain_offset_cm);
        }

        if (HAS_VALID_TARGET) {
            // Will the new offset target cause the sub to ascend above RNGHOLD_DEPTH?
            float desired_z_cm = rangefinder_terrain_offset_cm + rangefinder_target_cm;
            if (desired_z_cm >= sub.g.rnghold_depth) {
                // Adjust the terrain offset to stay below RNGHOLD_DEPTH, this should avoid "at_surface" events
                rangefinder_terrain_offset_cm += sub.g.rnghold_depth - desired_z_cm;
            }
